...
FORWARD _PROTOTYPE( int do_sys_change_gr, (message *m) );
FORWARD _PROTOTYPE( int do_sys_get_gr, (message *m) );
FORWARD _PROTOTYPE( int do_sys_change_qua, (message *m) );
FORWARD _PROTOTYPE( int do_sys_get_qua, (message *m) );
...
/* Main entry point of sys_task.  Get the message and dispatch on type. */
  ...
  while (TRUE) {
	...
	switch (m.m_type) {	/* which system call */
	    ...
	    case SYS_GETGR:	r = do_sys_get_gr(&m);      break;
	    case SYS_CHANGEGR:	r = do_sys_change_gr(&m);   break;
	    case SYS_GETQUA :	r = do_sys_get_qua(&m);      break;
	    case SYS_CHANGEQUA:	r = do_sys_change_qua(&m);   break;
 ...
 PRIVATE int do_sys_change_gr(message *m)
{
	...
	{
		tmp->group = 0;
		printf("The group has been changed to 0(A)\n");
	}
	else if(m->m1_i2 == 1)
	{
		tmp->group = 1;
		printf("The group has been changed to 1(B)\n");
	}
	else if(m->m1_i2 == 2)
	{
		tmp->group = 2;
		printf("The group has been changed to 2(C)\n");
	}
	return(OK);
}

PRIVATE int do_sys_get_gr(message *m)
{
	register struct proc *result, *iter;
	result = NIL_PROC;
	for(iter = BEG_USER_ADDR; iter < END_PROC_ADDR; ++iter)
	{
		if(iter->p_pid == m->m1_i1)
		{
			result = iter;
			break;
		}
	...
	if(result == NULL)
	{
		printf("Pid %d isn't found", m->m1_i1);
		return -1;
	}
	printf("The group of pid %d is %d\n", m->m1_i1, result->group);
	return result->group;
}

PRIVATE int do_sys_change_qua(message *m)
{
	register struct proc *iter;
	if(m->m1_i1 == 0)
	{
	  QUANT_A = m->m1_i2;
	}
	...
	printf("The quantum of group %d has been changed to %d\n", m->m1_i1, m->m1_i2);
	return(OK);
}

PRIVATE int do_sys_get_qua(message *m)
{
	m->m1_i2 = 0;
	if(m->m1_i1 == 0)
	{
	  m->m1_i2 = QUANT_A;
	...
	else if(m->m1_i1 == 1)
		{
		  m->m1_i2 = QUANT_B;
		       ...
		       m->m1_i2 = QUANT_C;
	printf("The quantum of group %d is %d\n", m->m1_i1, m->m1_i2);
	return m->m1_i2;
}
