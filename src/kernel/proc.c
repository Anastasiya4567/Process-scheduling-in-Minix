 ...
 */
  ...
  register struct proc *iter, *curr, *befA, *befB, *befC,  *temp_pointer; /*iterator, current process, previous process before processes A, B, C, termopary pointer */
  ...
  if(iter->p_nextready == NIL_PROC) /*if there is only one user process, do it*/
 {
	pick_proc();
	return;
  }
  ...
  while(iter->p_nextready != NIL_PROC) /*find the first process of groups A,B and C */
 {
	  if(befA == NIL_PROC && iter->p_nextready->group == 0)
	  {
	     befA = iter;
	  }
	  else if(befB == NIL_PROC && iter->p_nextready->group == 1)
  	{
	    befB = iter;
  	}
   	else if(befC == NIL_PROC && iter->p_nextready->group == 2)
      {
  	  befC = iter;
      }
  ...
  /* temp_poiner should point at the process before the one to be done */
  if(curr->group == 0)
  {
	  if(befB != NIL_PROC) 
		temp_pointer = befB;
	  else if(befC != NIL_PROC) 
	          temp_pointer = befC;
	  	else 
		     temp_pointer = befA;
  }
  else if(curr->group == 1)
  	{
	  if(befC != NIL_PROC) 
		temp_pointer = befC;
	  else if(befA != NIL_PROC) 
		 temp_pointer = befA;
	  	else 
		   temp_pointer = befB;
  	}
  	else
 	{
	  if(befA != NIL_PROC) 
		temp_pointer = befA;
	  else if(befB != NIL_PROC) 
		temp_pointer = befB;
	  	else 
		   temp_pointer = befC;
  	}

  iter = rdy_head[USER_Q]->p_nextready; /* remember the next process in queue under */

  rdy_tail[USER_Q]->p_nextready = rdy_head[USER_Q];
  rdy_tail[USER_Q] = rdy_head[USER_Q];
  rdy_head[USER_Q] = temp_pointer->p_nextready; /* set temp_pointer as new head */
  temp_pointer->p_nextready = temp_pointer->p_nextready->p_nextready; /* delete new head from it's old place */
  if(rdy_head[USER_Q] != iter) /* move iter to the next place after head in oder not to lose elements before temp_pointer */
  {
	 rdy_head[USER_Q]->p_nextready = iter;
  }
  rdy_tail[USER_Q]->p_nextready = NIL_PROC;
