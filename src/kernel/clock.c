 ...
 */
  ...
  if (--sched_ticks == 0)
  {
	...
	if(rdy_head[USER_Q]->group == 0)
		sched_ticks = QUANT_A * SCHED_RATE;
	else if(rdy_head[USER_Q]->group == 1)
		   sched_ticks = QUANT_B *SCHED_RATE;
	     else if(rdy_head[USER_Q]->group == 2)
			sched_ticks = QUANT_C * SCHED_RATE;
		  else 
			sched_ticks = SCHED_RATE;  /*reset quantum*/ 
	prev_ptr = bill_ptr;
