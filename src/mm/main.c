...
PUBLIC int do_get_gr()
{
	message msg;
	msg = mm_in;
	_taskcall(SYSTASK, SYS_GETGR, &msg);
	return 0;
}

PUBLIC int do_change_gr()
{
	message msg;
	msg = mm_in;
	return _taskcall(SYSTASK, SYS_CHANGEGR, &msg);
}

PUBLIC int do_change_qua()
{
	
	message msg;
	msg = mm_in;
	return _taskcall(SYSTASK, SYS_CHANGEQUA, &msg);
}

PUBLIC int do_get_qua()
{
	message msg;
	msg = mm_in;
	_taskcall(SYSTASK, SYS_GETQUA, &msg);
	return 0;
}
